...
////存储用户信息
//+ (void)saveUserInfoDefault:(AccountObject *)user{
//    NSLog(@"%@",user);
...
//    }
//    else{
//        user = [AccountObject mj_objectWithKeyValues:userDic];
//        return user;
//    }
//}
...
+ (NSString *)timeStrChangeToTimeNumWith:(NSString *)timeFormatter timeStr:(NSString *)timeStr{
    ...
    [formatter setDateFormat:timeFormatter]; //@"YYYYMMdd HHmmss" 设置你想要的格式,hh与HH的区别:分别表示12小时制,24小时制
    NSDate *date1 = [formatter dateFromString:timeStr]; //将字符串按formatter转成nsdate
    NSString *timeNum = [NSString stringWithFormat:@"%ld", (long)[date1 timeIntervalSince1970]];
    return timeNum;
}
//时间戳转时间
+ (NSString *)timeNumChangeToTimeStrWith:(NSString *)timeFormatter timeNum:(NSString *)timeNum{
    NSDateFormatter *formatter = [[NSDateFormatter alloc] init];
    [formatter setDateFormat:timeFormatter];//yyyy-MM-dd HH:mm:ss设置你想要的格式,hh与HH的区别:分别表示12小时制,24小时制
    NSString *timeStr = [NSString stringWithFormat:@"%@",[formatter stringFromDate:[NSDate dateWithTimeIntervalSince1970:[timeNum integerValue]]]];
    return timeStr;
}
@end
